...
import CryptoHelper from '../src/services/CryptoHelper'
import { ApiProps, Concrete } from '../src/models/types'
import { HTTP_Ok } from '../src/models/MgmExceptionTypes'
import { IAceConfiguration, IAceLoginRequest } from '../src/ace/interfaces'
import { MgmLogger } from '../src/services/MgmLogger'
import { HttpHandler, HttpResponse, http } from 'msw'
import { setupServer } from 'msw/node'
import { JwtHeader, SignOptions } from 'jsonwebtoken'
import { JwtSign } from '../src/services/mgm-jwt'
...
const aceConfigProd: Concrete<IAceConfiguration> = {
  MgmDebugLevel: 1,
  UseMgmJwtForPlayerId: false,
  logger: {
    logFileName: 'PIN Reset',
    logLevelDefault: 'all',
  },
  ...
  ace: {
    apiBaseUrl:
      'https://pinreset.mgmresorts.com/v2.3/bearer/PlayerTracking.svc',
    CrmAcresPinCheck: true,
    CrmAcresBaseUrl: 'https://pinresetaceprod.mgmresorts.com',
    adminLogin: {
      // This account is the admin for making changes to player PINs.
      UserName: 'ACE_REST_API',
      Password: 'p0ssw@rD%',
    ...
    pinResetSettings: {
      birthdayYear: {
        check: false,
        errorMessage:
          'We disallow using your birthday as a PIN for security reasons.',
      },
      previousPin: {
        check: true,
        errorMessage:
          'You may not reuse a previous PIN. Please try again with a new PIN.',
      },
      beginWithZero: {
        check: false,
      ...
      consecutiveNumbers: {
        check: false,
        errorMessage:
          'Consecutive numbers are not allowed for security reasons.',
      },
      easilyGuessable: {
        check: false,
...
const aceConfigQa4: Concrete<IAceConfiguration> = {
  MgmDebugLevel: 1,
  UseMgmJwtForPlayerId: false,
  logger: {
...
export function getAceConfig(useProduction = false) {
  return useProduction ? aceConfigProd : aceConfigQa4
}
...
export function getAceAdminLoginRequest() {
  const loginRequest: IAceLoginRequest = { ...getAceConfig().ace.adminLogin }

  return loginRequest
}

export function getNewMgmLogger() {
  const logger = new MgmLogger(
    'ACE REST Test',
    getAceConfig().logger.logFileName,
    getAceConfig().logger.logLevelDefault
  )
