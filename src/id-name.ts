...
export interface IdNameValueType<Tvalue, Type = string, Tid = string> extends IdName<Tid, string>, INameValue<Tvalue>, IType<Type> { }

/**
 * Used for sending data to a parent handler in a structured way with contextual id and name.
 */
export interface IValueChange<Tvalue = string> extends IdNameValueType<Tvalue, string, string>, IDate<number> { }
/**
...
export function createValueChange<T>(id: string, name: string, value: T, type?: string, date?: number) {
