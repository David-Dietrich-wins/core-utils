...
`)
  ...
  it('messageSuccessFail good without prefix', () => {
    ...
    expect(truncatedMsg).toBe(stringResultShouldStartWith)
  })
...
`)
    ...
    expect(msgTotalProcessed).toBe('1 suffix (Success: 0, Fail: 0)')

    istats.addProcessed(arrMessages)
    msgTotalProcessed = istats.messageTotalProcessedWithSuccessFail(true, true)

    expect(msgTotalProcessed).toBe('2 suffixes (Success: 0, Fail: 0)')
  })
})
...
describe('times', () => {
  ...
  describe('processedTimesArray', () => {
    ...
    it.each([
      ...
      const [
        ...
        avgRecordsPerSecond = totalProcessed / secondsToAdvance
  ...
  it.each([
    ...
    [204382, 0],
    [98456817, 0],
  ])('0 seconds %s, %s', (totalProcessed, secondsToAdvance) => {
    expect.assertions(4)
    ...
    const [
      ...
      avgRecordsPerSecond = totalProcessed / 1
